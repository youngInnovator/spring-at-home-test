Feature: Verify Artist Operations
  ...
  Scenario Outline: Find all artist using
    Given Artist exist in system with firstName "yarti" lastName "grad" category "ACTOR" dateOfBirth "2021-12-10"
    And Artist exist in system with firstName "testr" lastName "grad" category "ACTOR" dateOfBirth "2021-11-10"
    And Artist exist in system with firstName "resty" lastName "yart" category "PAINTER" dateOfBirth "2021-11-10"
    When I send request to find all artists
    Then the response will return status 200
    And result will contain 3 enteries
    Examples:
  ...
  Scenario Outline: Delete existing artist
